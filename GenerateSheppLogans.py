...
# Suggest sigma = 0.1, plotwindow = [0.9 1.1]
    ...
    for i in range(Nimg):
        ...
        X[:,:,i] = ((X[:,:,i]-Xmin)/(Xmax-Xmin))#normalization
        
        X2list = X[:,:,i].flatten().tolist()
...
# print(intensity)
np.save('/home/lvli/Documents/incident/sl/phantom.npy', phantom1[:,:,0].astype(np.float32))
phantom1.astype('float32').tofile('/home/lvli/Documents/incident/sl/phantom.bin')
np.save('/home/lvli/Documents/incident/sl/intensity.npy',np.array(intensity).astype(np.float32))
...
l = len(intensity1)
intensity1 = intensity1.astype(np.float32)
materials = ['Air']*l
...
activity_range_phantomD2 = DataFrame([intensity1[1:], intensity1[1:], (100*intensity1[1:]).astype(np.int8)]).T.astype('float32')
